...
const listkey = ["penTagram", "penTagram", "Api"];
...
router.get('/darkjokes', async (req, res, next) => {
        var apikeyInput = req.query.apikey
	if(!apikeyInput) return res.json(loghandler.notparam)	
	if (apikeyInput != 'penTagram')  return res.json(invalidKeynya)
       fetch(encodeURI(`https://raw.githubusercontent.com/penTagram/database-api/main/meme/darkjokes.json`))
        .then(response => response.json())
        .then(data => {
        var result = data;
        var result = data[Math.floor(Math.random() * data.length)];
